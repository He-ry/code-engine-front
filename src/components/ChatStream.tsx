...
export const ChatStream: React.FC<ChatStreamProps> = ({ messages, isGenerating, onSelectOption }) => {
  const { t } = useSettings();
        ...
        if (isUser) {
          return (
            <div key={msg.id} className="flex flex-col items-end w-full">
              ...
              <div className="inline-block px-3.5 py-1.5 bg-[#e8e8e8] dark:bg-zinc-800 text-gray-900 dark:text-zinc-100 rounded-xl text-sm leading-relaxed max-w-[85%] break-words">
                {msg.text}
              </div>
            </div>
          );
          ...
          <div key={msg.id} className="group flex flex-col items-start w-full space-y-3">
            ...
            {msg.clarificationQuestions && msg.clarificationQuestions.length > 0 && (() => {
              ...
              if (submittedAnswersForMsg) {
                ...
                return (
                  <div className="w-full mt-2 rounded-xl border border-gray-200/90 dark:border-[#2a2a2a] bg-[#f6f6f8] dark:bg-[#171717] p-4 sm:p-5 text-xs sm:text-sm font-sans space-y-3.5 shadow-2xs">
                    {/* Header Bar */}
              ...
              return (
                <div className="w-full mt-2 rounded-xl border border-gray-200/90 dark:border-[#2a2a2a] bg-[#f6f6f8] dark:bg-[#171717] p-4 sm:p-5 text-xs sm:text-sm font-sans space-y-4 shadow-2xs">
                  {/* Header Bar */}
